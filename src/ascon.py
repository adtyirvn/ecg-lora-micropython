...
class Ascon:
    ...
    def ascon_initialize(self, S, k, rate, a, b, key, nonce):
        ...
        """
        iv_zero_key_nonce = self.to_bytes([k, rate * 8, a, b] + (20-len(key))*[0]) + key + nonce
        S[0], S[1], S[2], S[3], S[4] = self.bytes_to_state(iv_zero_key_nonce)
        if self.debug: self.printstate(S, "initial value:")
        ...
        S[3] ^= zero_key[3]
        S[4] ^= zero_key[4]
        if self.debug: self.printstate(S, "initialization:")
    ...
    def ascon_process_associated_data(self, S, b, rate, associateddata):
        ...
        S[4] ^= 1
        if self.debug: self.printstate(S, "process associated data:")
    ...
    def ascon_process_plaintext(self, S, b, rate, plaintext):
        ...
        for block in range(0, len(p_padded) - rate, rate):
            if rate == 8:
                ...
                ciphertext += (self.int_to_bytes(S[0], 8) + self.int_to_bytes(S[1], 8))
        ...
        if rate == 8:
            ...
            S[0] ^= self.bytes_to_int(p_padded[block:block+8])
            S[1] ^= self.bytes_to_int(p_padded[block+8:block+16])
            ciphertext += (self.int_to_bytes(S[0], 8)[:min(8,p_lastlen)] + self.int_to_bytes(S[1], 8)[:max(0,p_lastlen-8)])
        if self.debug: self.printstate(S, "process plaintext:")
        return ciphertext
    ...
    def ascon_process_ciphertext(self, S, b, rate, ciphertext):
        ...
        for block in range(0, len(c_padded) - rate, rate):
            if rate == 8:
                ...
                Ci = (self.bytes_to_int(c_padded[block:block+8]), self.bytes_to_int(c_padded[block+8:block+16]))
                plaintext += (self.int_to_bytes(S[0] ^ Ci[0], 8) + self.int_to_bytes(S[1] ^ Ci[1], 8))
                S[0] = Ci[0]
                S[1] = Ci[1]
        ...
        if rate == 8:
            ...
            Ci = (self.bytes_to_int(c_padded[block:block+8]), self.bytes_to_int(c_padded[block+8:block+16]))
            plaintext += (self.int_to_bytes(S[0] ^ Ci[0], 8) + self.int_to_bytes(S[1] ^ Ci[1], 8))[:c_lastlen]
            if c_lastlen < 8:
                S[0] = Ci[0] ^ (S[0] & c_mask) ^ c_padding1
            else:
                S[0] = Ci[0]
                S[1] = Ci[1] ^ (S[1] & c_mask) ^ c_padding1
        if self.debug: self.printstate(S, "process ciphertext:")
        return plaintext
    ...
    def ascon_finalize(self, S, rate, a, key):
        ...
        S[rate//8+2] ^= self.bytes_to_int(key[16:] + self.zero_bytes(24-len(key)))
        ...
        if self.debug: self.printstate(S, "finalization:")
        return tag
    ...
    def ascon_permutation(self, S, rounds=1):
        ...
        """
        assert(rounds <= 12)
        if self.debugpermutation: self.printwords(S, "permutation input:")
        for r in range(12-rounds, 12):
            # --- add round constants ---
            S[2] ^= (0xf0 - r*0x10 + r*0x1)
            if self.debugpermutation: self.printwords(S, "round constant addition:")
            # --- substitution layer ---
            S[0] ^= S[4]
            S[4] ^= S[3]
            S[2] ^= S[1]
            ...
            if self.debugpermutation: self.printwords(S, "substitution layer:")
            ...
            if self.debugpermutation: self.printwords(S, "linear diffusion layer:")
